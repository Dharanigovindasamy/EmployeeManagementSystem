...
public class DepartmentDaoImpl implements DepartmentDao {
    ...
    public String getDepartmentName(int departmentId) throws EmployeeException {
        ...
        try {
            transaction = session.beginTransaction();
            departmentName = session.createQuery("SELECT departmentName FROM Department WHERE departmentId = : departmentId and isRemoved = :isRemoved" , Department.class)
                                              .setParameter("departmentId", departmentId);
                                              .setParameter("isRemoved" , false).uniqueResult();
            ...
            throw new EmployeeException("Unable to get department name : " + departmentId , e);
        } finally {
            if (session != null) {
                session.close();
            }
